...
use opdev_engine::{TestReportInspection, execute, inspect_junit};
use serde::Serialize;
...
pub(crate) fn run(start: &Path, suite_id: &str, junit: &Path) -> Result<ExitCode> {
    ...
    receipt.diagnostics.push("One observed command attempt and a previously absent report only. Ignored inputs, external dependencies, producer-internal retries and CI identity are not verified; no core gate or ledger was updated.".into());
...
fn collect_report(path: &Path, receipt: &mut Receipt) {
    match fs::symlink_metadata(path) {
...
fn aggregate(receipt: &Receipt) -> Outcome {
